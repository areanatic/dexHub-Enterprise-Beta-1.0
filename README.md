...
DexHub tracks every capability as an individually toggleable feature in [`.dexCore/_cfg/features.yaml`](.dexCore/_cfg/features.yaml) — 86 features today, classified by status:

| Status | Count | What it means |
|---|---|---|
| `always_on` | 7 | Core infrastructure (DexMaster, SSOT compile, validate.sh, D1 state model, consent tracking) |
| `enabled` | 60 | Shipped and tested — covered by this README |
| `deferred` | 19 | Planned for 1.1 or later (Parser Pattern B raster pipeline, native Workflow-Runner backend, native daemon for inbox watcher) |
