...
| 14 | `14-knowledge-l2-tank-query.test.sh` | bash + sqlite3 + ruby | ~18 structural + functional (query script present + no-longer-stub, FTS5 BM25 keyword search returns ranked markdown, JSON format parses + has expected shape, --top N limits, --source-type filter includes/excludes correctly, empty-tank + no-match + missing-tank all graceful exit 0, multi-line content round-trips intact, features.yaml flipped enabled) | — | — |
| 15 | `15-l2-copilot-wire.test.sh` | bash + sqlite3 + ruby | ~15 structural + functional (6 scenarios: no-seed=no-inject, missing-tank=no-inject, empty-tank=no-inject, ingest+seed=chunks baked with seed query echoed + size grew + still within 35KB cap + drift-check green, remove-seed=injection-gone, cap-enforcement on oversized chunks) | — | — |
