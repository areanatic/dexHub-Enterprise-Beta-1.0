...
<check if="new or starting fresh">
  Check `draft_folder` for existing docs. Ask user if they have a Product Brief.
...
<check if="user selects option 2">
  <invoke-workflow>{project-root}/.dexCore/dxm/workflows/2-plan/ux/workflow.yaml</invoke-workflow>
  <action>Pass mode="integrated" with Level 1-2 context</action>

</check>

<check if="user selects option 3">
  <invoke-task>{project-root}/.dexCore/dxm/tasks/ai-fe-prompt.md</invoke-task>
