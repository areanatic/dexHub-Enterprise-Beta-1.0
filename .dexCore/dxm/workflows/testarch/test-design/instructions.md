...
```xml
<task id="dex/dxm/testarch/test-design" name="Risk and Test Design">
  <llm critical="true">
  ...
  <flow>
    ...
    <step n="2" title="Assess Risks">
      <action>Use `{project-root}/.dexCore/dxm/testarch/tea-index.csv` to load the `risk-governance`, `probability-impact`, and `test-levels` fragments before scoring.</action>
