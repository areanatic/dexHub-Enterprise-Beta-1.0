...
```xml
<task id="dex/dxm/testarch/automate" name="Automation Expansion">
  <llm critical="true">
  ...
  <flow>
    ...
    <step n="2" title="Expand Automation">
      ...
      <action>Use `{project-root}/.dexCore/dxm/testarch/tea-index.csv` to load fragments such as `fixture-architecture`, `selective-testing`, `ci-burn-in`, `test-quality`, `test-levels`, and `test-priorities` before proposing additions.</action>
