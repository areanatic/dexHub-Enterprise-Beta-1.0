...
This directory houses the per-command workflows used by the Test Architect agent (`tea`). Each workflow wraps the standalone instructions that used to live under `testarch/` so they can run through the standard DEX workflow runner.
...
- `workflow.yaml` – metadata consumed by the DEX workflow runner.
