...
2. Confirm `.dexCore/dxm/config.yaml` defines `project_name`, `draft_folder`, `dev_story_location`, and language settings.
...
| ------------------ | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Setup              | -                                                                         | Analyst `*product-brief`, PM `*plan-project`, Architect `*solution-architecture` | `{draft_folder}/product-brief*.md`, `PRD.md`, `epics.md`, `solution-architecture.md` |
...
| Phase             | Test Architect                                                      | Dev / Team                                                 | Outputs                                                 |
| ----------------- | ------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------- |
| Refresh Context   | -                                                                   | Analyst/PM/Architect rerun planning workflows              | Updated planning artifacts in `{draft_folder}`         |
...
This separation lets us expand the knowledge base without touching agent wiring and keeps every command remote-controllable via the standard DEX workflow runner. As navigation improves, we can add lightweight entrypoints or tags in the index without changing where workflows live.
