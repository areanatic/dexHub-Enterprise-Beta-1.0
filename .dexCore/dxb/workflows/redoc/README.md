...
**Module:** Dex Builder (dxb)

## Purpose

ReDoc is an intelligent documentation maintenance system for DEX modules, workflows, and agents. It uses a reverse-tree approach (leaf folders first, then parent folders) to systematically generate and update README.md files with technical writer quality output.

The workflow understands DEX conventions deeply and focuses documentation on distinctive features rather than explaining standard patterns, resulting in succinct, precise technical documentation.
...
- **Convention-Aware**: Loads DEX architecture patterns and only documents unique/distinctive aspects
...
- **Module path**: `.dexCore/dxm` (documents entire module: root, workflows, agents)
- **Workflows folder**: `dex/dxm/workflows` (documents all workflows)
- **Agents folder**: `dex/dxm/agents` (documents all agents)
- **Single workflow**: `dex/dxm/workflows/product-brief` (documents one workflow)
- **Single agent**: `dex/dxm/agents/prd-agent.md` (documents one agent)
...
1. **Initialize**: Load DEX conventions and validate target
...
- **Context-Aware**: Uses DEX convention knowledge to highlight only distinctive features
