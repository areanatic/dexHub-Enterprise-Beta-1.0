...
The Build Workflow is an interactive workflow builder that guides you through creating new DEX workflows with proper structure, conventions, and validation. It ensures all workflows follow best practices for optimal human-AI collaboration and are fully compliant with the DEX Core v6 workflow execution engine.
...
- **Convention Enforcement**: Ensures adherence to DEX workflow creation guide
...
The brainstorming phase invokes the DIS brainstorming workflow to:
...
`{project-root}/.dexCore/{{target_module}}/workflows/{{workflow_name}}/`
...
- Module to host the new workflow (dxm, dxb, dis, or custom)
...
- **v5.0.0** - Initial DEX Core v6 compatible version
...
- Review `/dex/dxb/workflows/create-workflow/workflow-creation-guide.md`
- Check existing workflows in `/dex/dxm/workflows/` for examples
- Validate against `/dex/dxb/workflows/create-workflow/checklist.md`
