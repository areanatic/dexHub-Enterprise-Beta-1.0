...
<critical>You MUST have already loaded and processed: {project_root}/.dexCore/dxb/workflows/convert-legacy/workflow.yaml</critical>
...
<ask>Which module should this belong to? (eg. dxm, dxb, dis, or custom)</ask>
...
<critical>IMPORTANT: All paths must use final DEX installation locations, not src paths!</critical>
<action>Show user the target location: {project-root}/.dexCore/{{target_module}}/{{item_type}}/{{item_name}}</action>
<action>Note: Files will be created in dex/ but all internal paths will reference {project-root}/.dexCore/ locations</action>
...
5. Handle dependencies and paths:
   ...
   - CRITICAL: All paths must use {project-root}/.dexCore/{{module}}/ NOT src/

<action>Generate the converted v5 agent YAML file (.agent.yaml)</action>
<action>Example path conversions:

- exec="{project-root}/.dexCore/{{target_module}}/tasks/task-name.md"
- run-workflow="{project-root}/.dexCore/{{target_module}}/workflows/workflow-name/workflow.yaml"
- data="{project-root}/.dexCore/{{target_module}}/data/data-file.yaml"
  </action>
  <action>Save to: dex/{{target_module}}/agents/{{agent_name}}.agent.yaml (physical location)</action>
...
<invoke-workflow>
  workflow: {project-root}/.dexCore/dxb/workflows/create-agent/workflow.yaml
  inputs:
...
<invoke-workflow>
  workflow: {project-root}/.dexCore/dxb/workflows/create-workflow/workflow.yaml
  inputs:
...
<invoke-workflow>
  workflow: {project-root}/.dexCore/dxb/workflows/create-module/workflow.yaml
  inputs:
...
<action>Save report to: {draft_folder}/conversion-report-{{date}}.md</action>
</step>
