...
The Build Agent workflow is an interactive agent builder that guides you through creating DEX Core compliant agents as YAML source files that compile to final `.md` during install. It supports three agent types: Simple (self-contained), Expert (with sidecar resources), and Module (full-featured with workflows).
...
- **Validation Built-In**: Ensures YAML structure and DEX Core compliance
...
- **YAML Source**: `{custom_agent_location}/{{agent_filename}}.agent.yaml` (default: `dex/agents/`)
- **Installation Location**: `{project-root}/.dexCore/agents/{{agent_filename}}.md`
- **Compilation**: Run the DEX Method installer and select "Compile Agents (Quick rebuild of all agent .md files)"

#### For Module Agents

- **YAML Source**: `src/modules/{{target_module}}/agents/{{agent_filename}}.agent.yaml`
- **Installation Location**: `{project-root}/.dexCore/{{module}}/agents/{{agent_filename}}.md`
...
agent:
  metadata:
    id: dex/{{module}}/agents/{{agent_filename}}.md
    name: { { agent_name } }
    title: { { agent_title } }
    icon: { { agent_icon } }
    module: { { module } }
...
`{project-root}/.dexCore/_cfg/agents/{{module}}-{{agent_filename}}.customize.yaml`
...
1. **Standalone Agents** (not part of a module)
   - Source: Created in your custom agent location (default: `dex/agents/`)
   - Installed to: `{project-root}/.dexCore/agents/`
   - Compilation: Run DEX Method installer and select "Compile Agents"

2. **Module Agents** (part of DXM, DXB, or custom modules)
   - Source: Created in `src/modules/{module}/agents/`
   - Installed to: `{project-root}/.dexCore/{module}/agents/`
   - Compilation: Automatic during module installation
...
# For all Dex components (includes agents)
npm run install:dex
...
npx dex-method build:agents        # Build standalone agents
npx dex-method build:modules        # Build module agents (with modules)

# Full rebuild
npx dex-method build:all           # Rebuild everything
```

## Requirements

- DEX Core v6 project structure
...
1. **Step -1**: Optional brainstorming session
   - Uses DIS brainstorming workflow with agent-specific context
...
1. Review example agents in `/dex/dxm/agents/` for patterns
...
- Review example agents in `/dex/dxm/agents/`
- Check agent documentation in this workflow folder
- Test with simple agents first, then build complexity
- Consult DEX Method v6 documentation

---

_Part of the Dex Method v6 - DXB (Dex Builder) Module_
