...
- [ ] `metadata.id` points to final compiled path: `dex/{{module}}/agents/{{agent}}.md`
- [ ] `metadata.module` matches the module folder (e.g., `dxm`, `dxb`, `dis`)
...
- [ ] Customize file (if created) located at `{project-root}/.dexCore/_cfg/agents/{{module}}-{{agent}}.customize.yaml`

## Build Verification

- [ ] Run compile to build `.md`: `npm run install:dex` → "Compile Agents" (or `dex install` → Compile)
- [ ] Confirm compiled file exists at `{project-root}/.dexCore/{{module}}/agents/{{agent}}.md`
