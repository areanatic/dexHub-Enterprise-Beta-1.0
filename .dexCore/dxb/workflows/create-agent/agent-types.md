# DEX Agent Types Reference

## Overview

DEX agents come in three distinct types, each designed for different use cases and complexity levels. The type determines where the agent is stored and what capabilities it has.

## Directory Structure by Type

### Standalone Agents (Simple & Expert)

Live in their own dedicated directories under `dex/agents/`:
...
Part of a module system under `dex/{module}/agents/`:
...
**Location:** `dex/agents/{agent-name}/`
...
**Location:** `dex/agents/{agent-name}/` with sidecar directory
...
**Location:** `dex/{module}/agents/`

**Characteristics:**

- Part of a DEX module (dxm, dxb, dis)
...
agent:
  ...
  persona:
    ...
    principles: ['...']
  critical_actions:
    - 'Load config from {project-root}/.dexCore/{module}/config.yaml'
  menu:
    - trigger: create-prd
...
<agent id="dex/dxm/agents/pm.md" name="Martin" title="Product Manager" icon="📋">
  ...
  <critical-actions>
    <i>Load config from {project-root}/.dexCore/{module}/config.yaml</i>
  </critical-actions>
  <menu>
    <item cmd="*help">Show numbered menu</item>
    <item cmd="*create-prd" run-workflow="{project-root}/.dexCore/dxm/workflows/prd/workflow.yaml">Create PRD</item>
    <item cmd="*validate" exec="{project-root}/.dexCore/core/tasks/validate-workflow.xml">Validate document</item>
    <item cmd="*exit">Exit</item>
  </menu>
</agent>
```
