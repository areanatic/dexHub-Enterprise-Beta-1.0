# DEX Agent Architecture Reference
...
- `id` - Unique path identifier (e.g., "dex/dxm/agents/analyst.md")
- `name` - Agent's name (e.g., "Jana", "Martin", "Helper")
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.dexCore/{module}/config.yaml and set variables</i>
...
```xml
<module-integration>
  <module-path>{project-root}/.dexCore/{module-code}</module-path>
  <config-source>{module-path}/config.yaml</config-source>
  <workflows-path>{project-root}/.dexCore/{module-code}/workflows</workflows-path>
</module-integration>
```
...
Example: `{config_source}:draft_folder`

### Path Construction

```
Good: {project-root}/.dexCore/{module}/agents/
...
```xml
<!-- Full path -->
<item cmd="*create-prd" run-workflow="{project-root}/.dexCore/dxm/workflows/prd/workflow.yaml">
...
```xml
<item cmd="*validate" exec="{project-root}/.dexCore/core/tasks/validate-workflow.xml">
  Validate document
</item>
```
...
```xml
<item cmd="*standup"
   exec="{project-root}/.dexCore/dxm/tasks/daily-standup.xml"
...
<item cmd="*run" exec="{project-root}/.dexCore/module/task.md">
...
```xml
<critical-actions>
  <i>Load into memory {project-root}/.dexCore/{module}/config.yaml</i>
  <i>Remember the users name is {user_name}</i>
  <i>ALWAYS communicate in {communication_language}</i>
</critical-actions>
```
...
```xml
<agent id="dex/{module}/agents/{name}.md"
       name="{Name}"
       title="{Title}"
       icon="{emoji}">
  <persona>...</persona>
