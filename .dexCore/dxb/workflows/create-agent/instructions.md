...
<critical>You MUST have already loaded and processed: {project_root}/.dexCore/dxb/workflows/create-agent/workflow.yaml</critical>
<critical>Study YAML agent examples in: {project_root}/.dexCore/dxm/agents/ for patterns</critical>
...
If yes:
<action>Invoke brainstorming workflow: {project-root}/.dexCore/core/workflows/brainstorming/workflow.yaml</action>
...
- "Which module system would this fit best with?" (dxm, dxb, dis, or custom)
- Store as {{target_module}} for path determination
- Agent will be saved to: dex/{{target_module}}/agents/

For Simple/Expert agents (standalone):

- "This will be your personal agent, not tied to a module"
- Agent will be saved to: dex/agents/{{agent-name}}/
- All sidecar files will be in the same folder

<critical>Determine agent location:</critical>

- Module Agent → dex/{{module}}/agents/{{agent-name}}.agent.yaml
- Standalone Agent → dex/agents/{{agent-name}}/{{agent-name}}.agent.yaml
...
- **Agent name**: "Lisa", "Max", "Data Wizard" (personality-driven)
...
Example flow:
"So we have an analytical expert who helps with data... I'm thinking 'Lisa the Data Analyst' with a 📊 icon? Or maybe something more playful like 'Data Wizard' with 🧙?"
...
agent:
  metadata:
    id: dex/{{target_module}}/agents/{{agent_filename}}.md
...
<action>Check if DEX build tools are available:</action>

<check>If in DEX-METHOD project with build tools:</check>
