...
<critical>You MUST have already loaded and processed: {project_root}/.dexCore/dxb/workflows/create-module/workflow.yaml</critical>
<critical>Study existing modules in: {project_root}/.dexCore/ for patterns</critical>
...
If create:
<action>Invoke module-brief workflow: {project-root}/.dexCore/dxb/workflows/module-brief/workflow.yaml</action>
...
2. **Module code** - Generate kebab-case from name:
   ...
   Present as: _"Based on what you described, I suggest the module code: `{{proposed-code}}`. This will be used in paths like dex/{{proposed-code}}/agents/. Does this work or would you prefer something different?"_
...
- Module will be created at {installer_draft_folder}
...
# Module paths
module_root: "{project-root}/.dexCore/{{module_code}}"
installer_path: "{project-root}/.dexCore/{{module_code}}"
...
draft_folder: "{project-root}/docs/{{module_code}}"
...
install_steps:
  - name: 'Create directories'
    action: 'mkdir'
    paths:
      - '{project-root}/.dexCore/{{module_code}}'
      - '{project-root}/.dexCore/{{module_code}}/data'
      - '{project-root}/.dexCore/{{module_code}}/agents'

  - name: 'Copy configuration'
    action: 'copy'
    source: '{installer_path}/config.yaml'
    dest: '{project-root}/.dexCore/{{module_code}}/config.yaml'

  - name: 'Register module'
    action: 'register'
    manifest: '{project-root}/.dexCore/_cfg/manifest.yaml'
...
The module can be configured in `dex/{{module_code}}/config.yaml`
...
2. Run the DEX Method installer to this project location
...
5. Test module with: `dex install {{module_code}}`
