...
The Build Module workflow is an interactive scaffolding system that creates complete DEX modules with agents, workflows, tasks, and installation infrastructure. It serves as the primary tool for building new modules in the DEX ecosystem, guiding users through the entire module creation process from concept to deployment-ready structure.
...
The workflow loads critical variables from the DXB configuration:

- **custom_module_location**: Where custom modules are created (default: `dex/`)
...
- **Module Directory**: Complete module structure at `{project-root}/.dexCore/{module_code}/`
...
2. **Directory Structure** - Proper DEX module hierarchy
...
- **DEX Core Configuration** - Properly configured DXB config.yaml
...
2. **Review Existing Modules** - Study similar modules in `/dex/` for patterns and inspiration
...
2. **Test Installation** - Validate installer with `dex install {module_code}`
...
- Review the workflow creation guide at `/dex/dxb/workflows/create-workflow/workflow-creation-guide.md`
- Study module structure patterns at `module-structure.md`
- Validate output using `checklist.md`
- Consult existing modules in `/dex/` for examples

---

_Part of the Dex Method v5 - DXB (Builder) Module_
