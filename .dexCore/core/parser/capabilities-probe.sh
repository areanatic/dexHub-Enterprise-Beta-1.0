...
NEW_YAML=$(printf '%s' "$PROBE_RESULTS" | ruby -rjson -e '
  ...
  probe_results.each do |r|
    ...
    existing_backends[id]["version"]    = r["version"].nil? || r["version"].empty? ? "null" : r["version"].to_s.inspect
    existing_backends[id]["compliance"] = r["compliance"] || "ok"
    existing_backends[id]["last_probe"] = Time.now.utc.strftime("%Y-%m-%dT%H:%M:%SZ").inspect
    existing_backends[id]["probe_status"] = r["status"].to_s.inspect
