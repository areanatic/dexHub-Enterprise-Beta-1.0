# DEX Init - System Initialization Instructions

<workflow>

<step n="1" goal="Welcome and Status Check">
  <action>Display welcome banner with DEX branding</action>
  <action>Check for DEX installation at {project-root}/dex</action>
  <check>If installation found:</check>
    <action>Display current version from {project-root}/.dexCore/_cfg/manifest.yaml</action>
    <action>Show installation date and status</action>
  <check>If not found:</check>
    <action>Display warning that DEX is not installed</action>
...
<step n="2" goal="Present Initialization Options">
  ...
  <ask>Select an initialization task:
    ...
    1. Customize Installed Agents and Agent Party (Coming Soon)
       ...
       - NOTE: This can all be done manually, but doing it through here will be easier and also update the council-mode manifest
...
<action>Display message: Exiting DEX Init. Thank you!</action>
