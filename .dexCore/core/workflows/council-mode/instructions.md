# Council Mode - Multi-Agent Expert Council Instructions

<critical>The workflow execution engine is governed by: {project_root}/.dexCore/core/tasks/workflow.xml</critical>
<critical>This workflow orchestrates group discussions between all installed DEX agents</critical>
...
<action>Filter agent roster to exclude management agents:</action>
  - EXCLUDE agents where name = "mydex-agent" (UI/profile management, not discussion expert)
  - EXCLUDE agents where name = "mydex-project-manager" (project organization, not discussion expert)
  - These agents are for workspace management, not collaborative discussions

<action>For each remaining agent in filtered manifest:</action>
...
<step n="2" goal="Initialize Council Mode">
  <action>Announce council mode activation</action>
  <action>List all participating agents with their merged information:</action>
  <format>
    🏛️ COUNCIL MODE ACTIVATED 🏛️
    ...
    ──────────────────────────────────────
    💡 So startest du: Stelle eine Frage oder beschreibe dein Thema
    🎯 Beispiel: "Wie sollte ich meine API architekturieren?"
    🏛️ Die relevanten Agents antworten automatisch
    📖 Beenden: Tippe "*exit" oder "end council mode"
    ──────────────────────────────────────
...
<step n="3" goal="Orchestrate Multi-Agent Discussion" repeat="until-exit">
  ...
  <substep n="3c" goal="Handle Questions and Interactions">
    ...
    <check>If discussion becomes circular or repetitive:</check>
      <action>The Dex Master will summarize</action>
  ...
  <substep n="3e" goal="Check for Exit Conditions">
    <check>If user message contains any {{exit_triggers}}:</check>
      ...
      <goto step="4">Exit council mode</goto>

    <check>If user seems done or conversation naturally concludes:</check>
      <ask>Would you like to continue the discussion or end council mode?</ask>
...
<step n="4" goal="Exit Council Mode">
  ...
  <format>
    ...
    🏛️ Council Mode ended. Thanks for the great discussion!
...
<moderation>
  <note>If discussion becomes circular, have dex-master summarize and redirect</note>
